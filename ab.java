...
public class ab { 
  ...
  */ 
  ...
  /** 
   Buscar: homonimia de funciones (mismo nombre diferente no. de parametros), una busqueda RECURSIVA a partir de un nodo, recursivamente buscamos un dato,
  ...
  */ 
  private Nodo insertar(Nodo nodo, int dato) { 
    ...
    private Nodo father(Nodo nodo, int dato){
	...
	if(dato <= nodo.dato)
	    return father(nodo.izq,dato);
	    else
		return father(nodo.der,dato);
	...
	if (nodo.izq == null && nodo.der == null){
            tmp = nodo.dato;
	    father(nodo,nodo.dato).izq = null;
	    father(nodo,nodo.dato).der = null;
	    return tmp;
	}
   ...
   public void borrar(int dato){
	...
	if (curtmp.dato == dato && curtmp.izq == null && curtmp.der == null && curtmp == raiz){ //Si el dato esta en la raiz y la raiz no tiene hijos
	    ...
	    father(curtmp,dato).izq = null;
	    father(curtmp,dato).der = null;
       	    nodo = null;
	    return;
		}; 
	...
	if (curtmp.dato == dato && curtmp.der != null && curtmp.izq == null) // Si es un nodo interior con hijo derecho
	    {
		father(curtmp,dato).der = curtmp.der;
		return;
	    }

	if (curtmp.dato == dato && curtmp.izq != null && curtmp.der == null) // Si es un nodo interior con hijo izquierdo
	    {
		if(father(curtmp,dato).dato < curtmp.dato)
		    {
			father(curtmp,dato).der = curtmp.izq;
			return;
		    }

		if(father(curtmp,dato).dato >= curtmp.dato)
		    {
			father(curtmp,dato).izq = curtmp.izq;
			return;
		    }
	    }
